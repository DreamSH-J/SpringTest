package me.hackerj.springbootdeveloper.jwt;

import io.jsonwebtoken.Jwts;
import me.hackerj.springbootdeveloper.config.jwt.JwtProperties;
...
public class TokenProviderTest {
    ...
    void generateToken() {
        ...
        //then : jjwt 라이브러리를 사용해 토큰을 복호화함. 토큰을 만들 때
        //        클레임으로 넣어둔 id 값이 given 절에서 만든 유저 ID와 동일한지 확인
    ...
    void validToken_invalidToken() {
        ...
        //        이때 만료 시간은 1970년 1월 1일부터 현재 시간을 밀리초 단위로 치환한 값(new Date().getTime())에 1000을 빼,
        //        이미 만료된 토큰으로 생성
        String token = JwtFactory.builder()
                .expiration(new Date(new Date().getTime() - Duration.ofDays(7).toMillis()))
                .build()
                .createToken(jwtProperties);
        //when : 토큰 제공자의 validToken() 메소드를 호출해 유효한 토큰인지 검증한 뒤 결괏값을 반환받음
    ...
    void validToken_validToken() {
        ...
        //then : 반환 값이 true(유효한 토큰)인 것을 확인
    ...
    void getAuthentication() {
        //given : jjwt 라이브러리를 사용해 트큰을 생성
        //        이때 토큰의 제목인 subject는 "user@gmail.com" 라는 값을 사용
        ...
        assertThat(((UserDetails) authentication    // 인증에 필요한 모든 정보를 갖고 있음(authentication)
                .getPrincipal())
                .getUsername())
                .isEqualTo(userEmail);
    }
    ...
    void getUserId() {
        ...
        //when : 토큰 제공자의 getUserId() 메서드를 호출해 유저 ID룰 반환받음
